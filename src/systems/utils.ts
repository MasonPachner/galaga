...
export class Utils {
    ...
    public static distBetween(loc1, loc2) {
    ...
    public static wrapLocation(location) {
        if (location.x < -5) {
    ...
    public static angleBetween(loc1, loc2) {
        return Math.atan2(
            (loc2.y - loc1.y),
            (loc2.x - loc1.x)
        );
    ...
    public static bound(int, limit) {
        int = Math.min(int, limit);
        int = Math.max(int, -limit);
        return int;
    }

    public static pickStartLocation(direction) {
    ...
    public static closestObjectDistance(loc1, list) {
